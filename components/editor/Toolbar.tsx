...
const Toolbar: React.FC<ToolbarProps> = ({ editMode, isImageLoaded, onUpload, onModeChange, onHome }) => {
    return (
        <aside className="w-24 bg-[var(--bg-panel)] p-2 flex flex-col items-center space-y-4 shadow-lg z-10 overflow-y-auto no-scrollbar border-r border-[var(--bg-secondary)] transition-colors duration-300">
            ...
            <ToolbarButton icon="wand" label="AI Edit" onClick={() => onModeChange('ai-edit')} disabled={!isImageLoaded} isActive={editMode === 'ai-edit'}/>
